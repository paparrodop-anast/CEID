...
public class Image_Application extends JFrame implements ActionListener{
    ...
    int double_width, double_height;
    ...
    String picture_path=new String("icon_01.jpg");
...
double_option=new JMenuItem("Double");
double_option.setActionCommand("double");
...
public BufferedImage Double_Image(BufferedImage original_image, int type, int new_width, int new_height){
    double_width=2*new_width;
    double_height=2*new_height;
    
    temp_width=double_width;
    temp_height=double_height;
    
    BufferedImage doubled_image=new BufferedImage(double_width, double_height, type);
    Graphics2D graph_image=doubled_image.createGraphics();
    
    graph_image.drawImage(original_image, 0, 0, double_width, double_height, null);
    graph_image.dispose();
...
public void actionPerformed(ActionEvent e){
    if("align_left".equals(e.getActionCommand())){
        ...
        BufferedImage resized_image=Resize_Image(image, type,
        ...
        picture_label.setPreferredSize(new
        Dimension(Integer.parseInt(text_width.getText()),
        Integer.parseInt(text_height.getText())));
        ...
        picture_label.removeAll();
        image_panel.removeAll();
        BufferedImage doubled_image=Double_Image(image, type, temp_width, temp_height);
        picture_label=new JLabel(new ImageIcon(doubled_image));
        picture_label.setPreferredSize(new Dimension(temp_width, temp_height));
        picture_label.revalidate();
        image_panel.add(picture_label);
        image_panel.revalidate();
        validate();
